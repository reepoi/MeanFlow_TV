...
def combined_loss(model, source_samples, target_samples, t, r):
    ...
    _, dudt = func.jvp(f, (x_interp, t, r), tangents_partial_t)
    u, nabla_x_u_dot_v = func.jvp(f, (x_interp, t, r), tangents_partial_x, create_graph=True)


    target_1 = (velocity - tau * (nabla_x_u_dot_v + dudt)).detach()
    loss_1 = torch.nn.functional.mse_loss(u, target_1)

    target_2 = (velocity - u - tau * dudt).detach()
